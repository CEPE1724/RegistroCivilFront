...
const SeccionB = forwardRef((props, ref) => {
	...
	const validateForm = () => {
		...
		if (!formData.tipoEmpresa || formData.tipoEmpresa == 0 || formData.tipoEmpresa == "Selecciona una opcion") {
			newErrors.tipoEmpresa = "Este campo es obligatorio";
			if (!showSnackbar) {
				enqueueSnackbar("Por favor, selecciona el tipo de empresa", { variant: "error" });
				showSnackbar = true;
			}
		}
		...
		if (!formData.fechaIngreso) {
			...
			if (!showSnackbar) {
				enqueueSnackbar("Por favor, selecciona la fecha de ingreso", { variant: "error" });
				showSnackbar = true;
			}
		} else {
			...
			if (isNaN(fechaIngreso.getTime()) || fechaIngreso > fechaActual) {
				...
				if (!showSnackbar) {
					enqueueSnackbar("La fecha de ingreso no puede ser futura o inválida", { variant: "error" });
					showSnackbar = true;
				}
			}
		}
		...
		if (!formData.tipoContrato || formData.tipoContrato == 0 || formData.tipoContrato == "Selecciona una opcion") {
			newErrors.tipoContrato = "Este campo es obligatorio";
			if (!showSnackbar) {
				enqueueSnackbar("Por favor, selecciona el tipo de contrato", { variant: "error" });
				showSnackbar = true;
			}
		}

		if (!formData.tipoSueldo || formData.tipoSueldo == 0 || formData.tipoSueldo == "Selecciona una opcion") {
			newErrors.tipoSueldo = "Este campo es obligatorio";
			if (!showSnackbar) {
				enqueueSnackbar("Por favor, selecciona el tipo de sueldo", { variant: "error" });
				showSnackbar = true;
			}
		}
		...
		if (!formData.departamento) {
			...
			if (!showSnackbar) {
				enqueueSnackbar("Por favor, escribe el departamento", { variant: "error" });
				showSnackbar = true;
			}
		}

		if (!formData.cargo || formData.cargo == 0 || formData.cargo == "Selecciona una opcion") {
		...
		if (!formData.diasPago || formData.diasPago < 1 || formData.diasPago > 31) {
			...
			if (!showSnackbar) {
				enqueueSnackbar("Por favor, escribe los días de pago entre 1 y 31", { variant: "error" });
				showSnackbar = true;
			}
		}

		if (!formData.provincia || formData.provincia == 0 || formData.provincia == "Selecciona una opcion") {
		...
		if (!formData.canton || formData.canton == 0 || formData.canton == "Selecciona una opcion") {
		...
		if (!formData.parroquia || formData.parroquia == 0 || formData.parroquia == "Selecciona una opcion") {
		...
		if (!formData.barrio || formData.barrio == 0 || formData.barrio == "Selecciona una opcion") {
