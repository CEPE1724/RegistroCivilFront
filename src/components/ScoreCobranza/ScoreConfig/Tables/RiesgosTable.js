    ...
    return (
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-200">
            <div className="p-8 border-b-2 border-gray-100 flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold text-gray-900">Cbo_Riesgos</h2>
                    <p className="text-gray-600 mt-1">Gestión de niveles de riesgo</p>
                </div>
                <button
                    onClick={onAdd}
                            ...
                            <tr key={item.id} className={idx % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                                ...
                                <td className="px-6 py-4 text-sm">
                                    <input
                                        type="number"
                                        value={item.peso}
                                        onChange={(e) => onUpdate(item.id, 'peso', e.target.value)}
                                ...
                                <td className="px-6 py-4 text-sm">
                                    <input
                                        type="number"
                                        value={item.desde}
                                        onChange={(e) => onUpdate(item.id, 'desde', e.target.value)}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-right"
                                        placeholder="0.00"
                                        step="0.01"
                                    />
                                ...
                                <td className="px-6 py-4 text-sm">
                                    <input
                                        type="number"
                                        value={item.hasta}
                                        onChange={(e) => onUpdate(item.id, 'hasta', e.target.value)}
                                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-right"
                                        placeholder="100.00"
                                        step="0.01"
                                    />
